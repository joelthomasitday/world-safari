        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {packages.map((pkg, idx) => (
              <ScrollReveal key={pkg._id} animation="reveal" delay={100 + idx * 100}>
                <Link 
                  href={`/packages/${pkg.slug}`}
                  className="group bg-white/5 border border-white/10 rounded-2xl overflow-hidden hover:border-white/20 hover:shadow-2xl hover:shadow-black/50 hover:-translate-y-1 transition-all duration-300 block"
                >
                  {/* Image */}
                  ...
                  <div className="p-6 md:p-8">
                    ...
                    <h3 className="text-2xl font-sans font-bold mb-2 group-hover:text-primary-foreground transition-colors">
                      {pkg.title}
                    </h3>
